sap.ui.define([
    "zcostgroupstep/controller/BaseController",
    "sap/ui/model/Sorter",
	"sap/ui/model/Filter",
	...
	"sap/m/Button",
	"sap/m/Dialog",
    "zcostgroupstep/sheetjs/jszip",
	"zcostgroupstep/sheetjs/xlsx.full.min"
],
    /**
    ...
    function (Controller, Sorter, Filter, FilterOperator, JSONModel, ODataModel, ResourceModel, Spreadsheet, MessageToast, MessageBox, SearchField, Button, Dialog) {
        ...
        return Controller.extend("zcostgroupstep.controller.Main", {
            ...
            setInitPage : function(oView){
                ...
                //view.xml table id
                _oTable = oView.byId("iCOSTGROUPSTEPList");
                
                //view.xml table row path model
                _oViewTableModel = this._createJSONModel(oView,"oZCOSTGROUPSTEP");
                ...
                //main model
                _oModelMain = this._setCustomModel(_oDataList, oView, "oMODELMAIN", "/sap/opu/odata/sap/YY1_ZCOSTGROUP_STEP_CDS/", "/YY1_ZCOSTGROUP_STEP", "ODataModel");
                ...
                $.when(     
                    this._getODataModelRead(_oDataList["oGLACCOUNT"],aFilter,null,sSorter)
                ).done(function(oResults){
                    oGLACCOUNT.setSizeLimit(100000);
                    oGLACCOUNT.setProperty("/", oResults);
                });

                var sSorter = [
                    new Sorter("COSTGROUP")
                ]
                $.when(     
                    this._getODataModelRead(_oDataList["oCOSTGROUP"],null,null,sSorter)
                ).done(function(oResults){
            ...
            onSearch : function(vEvent){
                ...
                $.when(     
                    ...
                    this._getODataModelRead(_oDataList["oGLACCOUNT"],aFilterVH),
                    this._getODataModelRead(_oDataList["oCOSTGROUP"])
                ).done(function(oResults,oVHGL,oVHCG){
                    ...
                    //코드, 명 매칭
                    oResults = self._setValueHelpName(oResults,oVHGL,"GLACCOUNT","GLAccount","GLACCOUNTNAME","GLAccountName");
            ...
            onPreSave : async function(){
                ...
                for(var i = 0; i <aSelectedRows.length; i++){
                    ...
                    if(!item.STEP)        return MessageToast.show(_i18n.getText("STEP")+_i18n.getText("MSG_PK_CHECK"));
                    if(!item.GLACCOUNT)   return MessageToast.show(_i18n.getText("GLACCOUNT")+_i18n.getText("MSG_PK_CHECK"));
                ...
                self._getODataModelRead(_oDataList["oMODELMAIN"]).done(function (aDataArr) {
                    ...
                    Promise.all(aDeleteCallArr).then(function () {
                        ...
                        aSelectedRows.forEach(function (item) {
                            ...
                            oData.STEP = item.STEP;
                            oData.GLACCOUNT = item.GLACCOUNT;
                ...
                aSelectedRows.forEach(function (item) {
                    ...
                    if(item.SAP_UUID){
                        ...
                        var aFilter = [
                            ...
                            new Filter("STEP", "EQ", item.STEP),
                            new Filter("GLACCOUNT", "EQ", item.GLACCOUNT),
            ...
            onSaveResult : function(aCallCBOERROR){
                ...
                aCallCBOERROR.forEach(function (item, idx) {
                    if(idx === 0){
                        returnMsg = _i18n.getText("STEP") + " : " + item.STEP + ", " + _i18n.getText("GLACCOUNT") + " : " + item.GLACCOUNT + ", " + _i18n.getText("COSTGROUP") + " : " + item.COSTGROUP;
                    }else{
                        returnMsg = returnMsg + "\n" + _i18n.getText("STEP") + " : " + item.STEP + ", " + _i18n.getText("GLACCOUNT") + " : " + item.GLACCOUNT + ", " + _i18n.getText("COSTGROUP") + " : " + item.COSTGROUP;
                    }
                });
            ...
            onCodeListCheck : function(oEvent, vField, vFieldName) {
                ...
                if(this[oDataName]) {
                    var aCodeList = this[oDataName] || [],
                        cCodeData = false, cCodeDataValue;
                    ...
                    if(cCodeData[sKey]){
                        cCodeDataValue = cCodeData[sKey];
                        if(vField === "GLACCOUNT"){
                            cCodeDataValue = self._conversionCodeAdd(cCodeData[sKey],10);
                        }
                    }

                    if(cCodeData) {
                        oEvent.getSource().getParent().oParent.mBindingInfos.rows.binding.oList[sRowModelPath][vField] = cCodeDataValue;
            ...
            onValueHelpDialog : function(oEvent, sGubunField, sName){
                var oInput = oEvent.getSource(), self=this,
                    nRowIndex = oEvent.getSource().getParent().getIndex(),
                    sRowModelPath = _oTable.getContextByIndex(nRowIndex).getPath().replace("/",""),
                    oParamer = {},
                    aFilter = [];
                ...
                this._openSearchDialog(oParamer, aFilter, function(e){
                    ...
                    var cCodeDataValue = oDialog.getTitle();
                    if(sGubunField === "GLACCOUNT"){
                        cCodeDataValue = self._conversionCodeAdd(oDialog.getTitle(),10)
                    }
                    oInput.setValue(cCodeDataValue);           
            ...
            onValueHelpRequested: function(oEvent, param) {
                ...
                if (param === 'fbGLACCOUNT') {
                    sDialogName = "GLAccount";
                    sModelName = "oGLACCOUNT";
                ...
                this.loadFragment({
                    name: "zcostgroupstep/view/fragment/" + sDialogName,
            ...
            onFilterBarSearch: function (oEvent) {
                ...
                var sPath1, sPath2;
                if (this._oVHD.getKey() === "GLAccount") {
                    sPath1 = "GLAccount";
                    sPath2 = "GLAccountName";
                } else if (this._oVHD.getKey() === "COSTGROUP") {
                    sPath1 = "COSTGROUP";
                    sPath2 = "COSTGROUPTEXT";
                } else if (this._oVHD.getKey() === "CompanyCode") {
                    sPath1 = "CompanyCode";
            ...
            excelProcess : function (data) {
                ...
                excelRows.forEach(function(item,idx){
                    ...
                    exData.COMPANYCODE_Text = item[_i18n.getText("COMPANYCODE_Text")];
                    exData.STEP = item[_i18n.getText("STEP")];
                    exData.GLACCOUNT = self._conversionCodeAdd(item[_i18n.getText("GLACCOUNT")],10);
                    exData.GLACCOUNTNAME = item[_i18n.getText("GLACCOUNTNAME")];
            ...
            excelTempletDown : function(){
                ...
                oDataSample.COMPANYCODE_Text = "";
                oDataSample.STEP = "1";
                oDataSample.GLACCOUNT = "13711100";
                oDataSample.GLACCOUNTNAME = "";
                oDataSample.COSTGROUP = "1001";
                oDataSample.COSTGROUPTEXT = "";
                oDataSample.USEYN = "X";
                ...
                oDataSample.COMPANYCODE_Text = "";
                oDataSample.STEP = "2";
                oDataSample.GLACCOUNT = "44401100";
                oDataSample.GLACCOUNTNAME = "";
            ...
            excelColumnTempConfig : function(){
                return [
                    ...
                    { label : _i18n.getText("COMPANYCODE_Text"), 	property : "COMPANYCODE_Text" },
                    { label : _i18n.getText("STEP"), 			    property : "STEP" },
                    { label : _i18n.getText("GLACCOUNT"), 			property : "GLACCOUNT" },
