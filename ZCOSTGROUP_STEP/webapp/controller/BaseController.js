sap.ui.define(
    ...
    function(BaseController, Sorter, Filter, FilterOperator, JSONModel, ODataModel, Spreadsheet, MessageToast, MessageBox, SearchField, Button, Dialog) {
      ...
      return BaseController.extend("zcostgroupstep.controller.App", {
        onInit: function() {
        ...
        _getSearchFilter : function(){
            ...
            aFilterItems.forEach(function(item){
                ...
                if(oFilter){
                    if(oFilter.aFilters != null){
                        if(oFilter.aFilters.length > 0){
                            // gl account 관련 필드인 경우 필터값 앞에 "00" 붙이기
                            oFilter.aFilters.forEach(function (item) {
                                if (vFieldName === "GLACCOUNT") {
                                    var sFilterValue = item.oValue1;
                                    item.oValue1 = "00" + sFilterValue;
                                }
                            })
                        }
                    }
         ...
         * 코드값 자릿수 리턴
         */
        _conversionCodeAdd : function(sCode,vInt){
        ...
        _conversionCodeDel : function(sCode){
            return sCode.replace(/^0+(?!$)/, '');
        },
        ...
        _setValueHelpName : function(tModel, sModel, tCode, sCode, tName, sName){
            ...
            for (const oModel of tModel) {
                for (const oValueHelp of sModel) {
                    if (self._conversionCodeDel(oModel[tCode]) === oValueHelp[sCode]) {
